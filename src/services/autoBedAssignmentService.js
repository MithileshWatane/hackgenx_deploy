...
export async function autoAssignBed(
  ...
  try {
        ...
        .insert([
          {
            ...
            confidence: 0.8,
            reasoning:
              "Initial estimated stay based on average admission duration",
          },
        ]);

      if (predictionError) {
        console.error(
          "Failed to create discharge prediction:",
          predictionError
        );
    ...
    let queuePosition =
      waitingPatients?.findIndex((p) => p.id === queueEntryId) ?? 0;
    if (queuePosition === -1) queuePosition = waitingPatients?.length || 0;

    const waitTimeMinutes = await calculateEstimatedWaitTime(
      bedType,
      queuePosition
    );
    console.log(
    ...
    const { error: updateError, data: updateData } = await supabase
      ...
      .update({
        ...
        notes: `Waiting for bed. Estimated wait: ${formatWaitTime(
          waitTimeMinutes
        )}`,
      })
      .eq("id", queueEntryId)
    ...
    return {
      ...
      bedAssigned: false,
      waitTimeMinutes,
      message: `No ${bedType} bed available. Estimated wait: ${formatWaitTime(
        waitTimeMinutes
      )}`,
    };
  } catch (error) {
...
async function calculateEstimatedWaitTime(
  ...
  try {
    ...
    for (const bed of occupiedBeds) {
      ...
      if (activeQueue?.predictions?.length > 0) {
        ...
        if (latestPrediction.remaining_days != null) {
          ...
          const dischargeDate = new Date(
            latestPrediction.predicted_discharge_date + "T00:00:00"
          );
          const now = new Date();
          remainingMinutes = Math.max(
            0,
            Math.ceil((dischargeDate - now) / (1000 * 60))
          );
        }
      }
      ...
      if (remainingMinutes === null) {
        ...
        const admittedAt = new Date(
          activeQueue?.bed_assigned_at || activeQueue?.admitted_at || Date.now()
        );
        const now = new Date();
    ...
    if (queuePosition < releaseTimes.length) {
      ...
      const avgRelease =
        releaseTimes.reduce((sum, t) => sum + t, 0) / releaseTimes.length;
      return Math.ceil(
        avgRelease + (queuePosition - releaseTimes.length + 1) * 30
      );
    }
  } catch (error) {
...
export async function assignSinglePatient(doctorId = null) {
  try {
    ...
    if (predictionError) {
      console.error(
        `Failed to create prediction for ${patient.patient_name}:`,
        predictionError
      );
    }
    ...
    if (updateDcError) {
      console.error(
        `Failed to update discharge time for ${patient.patient_name}:`,
        updateDcError
      );
    }
...
export async function processWaitingQueue(doctorId = null) {
  try {
    ...
    for (
      let i = 0;
      i < Math.min(waitingPatients.length, availableBeds.length);
      i++
    ) {
      ...
      if (queueError) {
        console.error(
          `Failed to assign bed to ${patient.patient_name}:`,
          queueError
        );
        continue;
      }
      ...
      if (bedUpdateError) {
        console.error(
          `Failed to update bed status for ${bed.bed_number}:`,
          bedUpdateError
        );
        continue;
      }
